...
const CustomMap = () => {
  ...
  const fetchRoute = async (locations, extraStopIndex = 0) => {
    try {
      ...
      const origin = {
        lat: routeLocations[0].latitude,
        lng: routeLocations[0].longitude
      };

      const destination = {
        lat: routeLocations[routeLocations.length - 1].latitude,
        lng: routeLocations[routeLocations.length - 1].longitude
      };
      ...
      const apiKey = process.env.EXPO_PUBLIC_ola_api;

      const apiUrl = `https://api.olamaps.io/routing/v1/directions?origin=${origin.lat},${origin.lng}&destination=${destination.lat},${destination.lng}&waypoints=${waypoints.map(wp => `${wp.lat},${wp.lng}`).join("|")}&api_key=${apiKey}`;
      ...
      if (data.routes && data.routes.length > 0) {
        ...
        // Extract route information from the first leg
        if (route.legs && route.legs.length > 0) {
          const firstLeg = route.legs[0];
          setRouteInfo({
            duration: Math.round(firstLeg.duration / 60), // Convert to minutes
            distance: Math.round(firstLeg.distance / 1000 * 100) / 100 // Convert to km with 2 decimals
          });
        }
